    ...
    if a==0:
        ...
        if response==1:
            if a==1:
                ...
                c5+=1
            ap.writerow([f" Hrishikesh:{c1}",f" Jeevitesh:{c2}",f" Manish:{c3}",f" Thayanitha:{c4}",f" Muhilan:{c5}",f" Total:{c1+c2+c3+c4+c5}",f" Time Stamp:{dt.datetime.now()}"])
            v.set(0)
...
txt=Label(root,text="Please click on the desired radio button to cast your vote").grid(row=0,columnspan=5)
imgs=[]
for i in range(0,5):
    img=ImageTk.PhotoImage(Image.open(f"pictures\\{i+1}.png"))
    imgs.append(img)
...
confirm_button=Button(root,text="Submit",padx=30,pady=40,command=submit,bg='red',fg='white').grid(row=3,columnspan=5)
exit_button=Button(root,text="Exit program",padx=30,pady=30,fg='white',bg='red',command=exit).grid(row=3,column=4,columnspan=5)
