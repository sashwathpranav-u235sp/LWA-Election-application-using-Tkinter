...
candidate_info={1:'Athsal',2:'Dishika Chandiramouli',3:"Sivaani S",4:'Pavithra Suresh',5:'R Sai Amritha',6:"Meenatche Sri",7:"Joelle Siyona",8:"Akshitha"}
c1=0
c2=0
c3=0
c4=0
    ...
    if a==0:
        ...
        if response==1:
            if a==1:
                ...
                c8+=1
            ap.writerow([f" {candidate_info[1]}:{c1}",f" {candidate_info[2]}:{c2}",f" {candidate_info[3]}:{c3}",f" {candidate_info[4]}:{c4}",f" {candidate_info[5]}:{c5}",f" {candidate_info[6]}:{c6}",f" {candidate_info[7]}:{c7}",f" {candidate_info[8]}:{c8}",f" Total:{c1+c2+c3+c4+c5+c6+c7+c8}",f" Time Stamp:{dt.datetime.now()}"])
            v.set(0)
...
txt=Label(root,text="Please click on the desired radio button to cast your vote").grid(row=0,columnspan=8)
imgs=[]
for i in range(0,8):
...
Radiobutton(root,text=candidate_info[8],variable=v,value=8,command=lambda:showvar(8),anchor='s').grid(row=2,column=7)
confirm_button=Button(root,text="Submit",padx=30,pady=40,command=submit,bg='red',fg='white').grid(row=3,columnspan=8)
exit_button=Button(root,text="Exit program",padx=30,pady=30,fg='white',bg='red',command=exit).grid(row=3,column=4,columnspan=8)
